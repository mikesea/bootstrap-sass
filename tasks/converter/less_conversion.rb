...
class Converter
  module LessConversion
    ...
    def process_stylesheet_assets
        ...
        case name
          ...
          when 'variables.less'
            ...
            file = replace_all file, %r{(\$icon-font-path): \s*"(.*)" (!default);},  "\n" + unindent(<<-SCSS, 14)
              ...
              \\1: if($bootstrap-sass-asset-helper, "bootstrap-3/bootstrap/", "\\2bootstrap-3/bootstrap/") \\3;
            SCSS
          when 'close.less'
          ...
          when 'glyphicons.less'
            file = bootstrap_font_files.map { |p| %Q(//= depend_on "bootstrap-3/bootstrap/#{File.basename(p)}") } * "\n" + "\n" + file
      ...
      save_file main_to, File.read(main_from).gsub(/ "/, ' "bootstrap-3/bootstrap/')
